...
* `subj_id`: identify the subjects. This labels the single data point within each `roi_id`.
* `roi_id`: brain sub-region that of interest for the analysis. We are interested in eight brain regions.
* `group`: whether the test was `Paired` condition or `Control` condition.
* `color_effect`: the value that indicate the memory trace of color. For each `subj_id` at each with each test condition `group` at each brain region`roi_id`, we obtained a single `color_effect` value.
